...
ipatch_dls2_info_set(IpatchDLS2Info **info, guint32 fourcc, const char *value)
{
    ...
    while(p)		  /* search for existing info with fourcc ID */
    {
        ...
        if(bag->fourcc == fourcc)
        {
            ...
            {
                bag->value = g_strdup(value);    /* set the value */
            }
    ...
    if(last)
    {
        last = g_slist_append(last, bag);    /* info list not empty? assign to keep gcc happy */
    }
    else
    {
        *info = g_slist_append(NULL, bag);
    }
}
