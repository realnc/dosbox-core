                                ...
                                GValue *value, GParamSpec *pspec)
{
    ...
    default:
        ...
        if(get_from_sample)	/* get sample info from linked sample? */
        {
            ...
            {
                ipatch_dls2_sample_info_get_property(NULL, property_id, value);
            }
        }
        else if(!retval)
        {
                             ...
                             gpointer user_data)
{
    ...
    if(refsample)
    {
        ipatch_dls2_region_set_sample(dest_reg, refsample);
    }
                                   ...
                                   IpatchDLS2SampleInfo *info)
{
    ...
    else if(region->sample)
    {
        sample = g_object_ref(region->sample);
    }
    else
    {
        info_set = FALSE;
    ...
    if(!info_set)
    {
        ipatch_dls2_sample_info_init(info);
    }
}
