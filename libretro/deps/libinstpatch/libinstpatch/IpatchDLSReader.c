...
restart: /* to restart parsing if a DLS file turns out to be GigaSampler */
    ...
    if(!ipatch_dls_reader_start(reader, err))
    {
        return (NULL);
    }
    ...
    if(!ipatch_dls_reader_fixup(reader, err))
    {
        return (NULL);
    }
...
ipatch_dls_reader_start(IpatchDLSReader *reader, GError **err)
{
    ...
    if(IPATCH_IS_GIG_FILE(riff->handle->file))
    {
        reader->is_gig = TRUE;
    }
    ...
    if(reader->is_gig)
    {
        reader->dls = IPATCH_DLS2(ipatch_gig_new());
    }
    else
    {
        reader->dls = ipatch_dls2_new();
    }
...
ipatch_dls_reader_fixup(IpatchDLSReader *reader, GError **err)
{
    ...
    while(i < reader->pool_table_size)
    {
        ...
        {
            g_warning("Invalid wave pool entry (index=%d)", i);
        }
    ...
    while((chunk = ipatch_riff_read_chunk(riff, err)))
    {
        if(chunk->type == IPATCH_RIFF_CHUNK_LIST)
        {
            ...
            case IPATCH_GIG_FOURCC_3GRI:
                if(!assert_loading_gig(reader, err))
                {
                    return (FALSE);
                }
...
ipatch_dls_reader_load_inst_list(IpatchDLSReader *reader, GError **err)
{
    ...
    while((chunk = ipatch_riff_read_chunk(riff, err)))
    {
                ...
                chunk->id == IPATCH_DLS_FOURCC_INS)
        {
            ...
            if(reader->is_gig)
            {
                inst = IPATCH_DLS2_INST(ipatch_gig_inst_new());
            }
            else
            {
                inst = ipatch_dls2_inst_new();
            }
            ...
            if(!ipatch_riff_get_error(riff, NULL))
            {
                return (FALSE);
            }
        }
                            ...
                            IpatchDLS2SampleInfo *info, GError **err)
{
            ...
            + IPATCH_DLS_WSMP_LOOP_SIZE)
    {
        ...
        {
            info->options |= IPATCH_SAMPLE_LOOP_STANDARD;    /* default */
        }
...
ipatch_dls_load_dlid(IpatchRiff *riff, guint8 *dlid, GError **err)
{
    ...
    if(!ipatch_file_read(riff->handle, dlid, 16, err))
    {
        return (FALSE);
    }
                                ...
                                IpatchGigRegion *region, GError **err)
{
    ...
    if(chunk->size == 0)
    {
        return (TRUE);    /* no dimension names */
    }
                            ...
                            GError **err)
{
    ...
    while((chunk = ipatch_riff_read_chunk(riff, err)))
    {
                ...
                && chunk->id == IPATCH_GIG_FOURCC_3GNM)
        {
            ...
            if(!ipatch_file_read(riff->handle, &buf, size, err))
            {
                goto err;
            }
