...
ipatch_dls_writer_finalize(GObject *object)
{
    ...
    if(writer->sample_postbl)
    {
        g_free(writer->sample_postbl);
    }
...
ipatch_dls_writer_new(IpatchFileHandle *handle, IpatchDLS2 *dls)
{
    ...
    if(handle)
    {
        ipatch_dls_writer_set_file_handle(writer, handle);
    }

    if(dls)
    {
        ipatch_dls_writer_set_patch(writer, dls);
    }
...
ipatch_dls_writer_save(IpatchDLSWriter *writer, GError **err)
{
    ...
    if(writer->dls)
    {
        g_object_unref(writer->dls);    /* shouldn't be set, but.. */
    }

    /* are we writing a GigaSampler file? */
    if(IPATCH_IS_GIG(writer->orig_dls))
    {
        writer->is_gig = TRUE;
    }
...
ipatch_dls_write_level_0(IpatchDLSWriter *writer, GError **err)
{
    ...
    {
        if(!dls_write_info(writer, writer->dls->info, err))
        {
            return (FALSE);
        }
    }
    ...
    if(writer->dls->dlid)
        if(!dls_write_dlid(writer, writer->dls->dlid, err))
        {
            return (FALSE);
        }
    ...
    if(!dls_write_inst_list(writer, err))
    {
        return (FALSE);
    }
    ...
    if(!dls_reserve_pool_table(writer, err))
    {
        return (FALSE);
    }
    ...
    if(!dls_write_wave_pool(writer, err))
    {
        return (FALSE);
    }
    ...
    if(!dls_fixup_pool_table(writer, err))
    {
        return (FALSE);
    }
...
gig_write_file_info(IpatchDLSWriter *writer, GSList *info_list, GError **err)
{
    ...
    for(i = 0; i < G_N_ELEMENTS(order); i++)
    {
        ...
        {
            size = IPATCH_GIG_MOST_INFO_SIZE;
        }
        ...
        if(!ipatch_riff_write_sub_chunk(riff, order[i], err))
        {
            goto err;
        }

        if(!ipatch_file_write(riff->handle, buf, size, err))
        {
            goto err;
        }
...
dls_write_inst_list(IpatchDLSWriter *writer, GError **err)
{
    ...
    while(inst)			/* loop over instruments */
    {
        ...
        if(writer->is_gig)
        {
            if(!gig_write_name_info(writer, inst->info, err))
            {
                return (FALSE);
            }
        ...
        else if(!dls_write_info(writer, inst->info, err))
        {
            return (FALSE);
        }
        ...
        /* write DLSID if its set */
        if(inst->dlid)
            if(!dls_write_dlid(writer, inst->dlid, err))
            {
                return (FALSE);
            }
        ...
        {
            retval = gig_write_region_list(writer, IPATCH_GIG_INST(inst), err);
        }
        ...
        if(inst->conns)
        {
            ...
            if(!dls_write_art_list(writer, inst->conns, err))
            {
                return (FALSE);
            }
                      ...
                      GError **err)
{
    ...
    while(region)		/* loop over regions */
    {
        ...
        if(region->info)
        {
            ...
            if(!dls_write_info(writer, region->info, err))
            {
                return (FALSE);
            }
        ...
        if(!dls_write_region_header(writer, region, err))
        {
            return (FALSE);
        }
        ...
        if(!dls_write_wave_link(writer, region, err))
        {
            return (FALSE);
        }
                      ...
                      GError **err)
{
    ...
    while(region)		/* loop over regions */
    {
        ...
        if(!gig_write_region_header(writer, region, err))
        {
            return (FALSE);
        }
        ...
        if(!gig_write_wave_link(writer, region, err))
        {
            return (FALSE);
        }
        ...
        if(!gig_write_sub_regions(writer, region, err))
        {
            return (FALSE);
        }
                      ...
                      GError **err)
{
    ...
    if(loop_type != IPATCH_SAMPLE_LOOP_NONE)
    {
        ...
        {
            saveloop_type = IPATCH_DLS_WSMP_LOOP_FORWARD;    /* default */
        }
...
dls_fixup_pool_table(IpatchDLSWriter *writer, GError **err)
{
    ...
    if(!ipatch_file_write(riff->handle, writer->sample_ofstbl,
                          writer->sample_count * 4, err))
    {
        return (FALSE);
    }

    /* seek back to original position */
    if(!ipatch_file_seek(riff->handle, retpos, G_SEEK_SET, err))
    {
        return (FALSE);
    }
...
dls_write_wave_pool(IpatchDLSWriter *writer, GError **err)
{
    ...
    for(index = 0; sample; index++, sample = ipatch_dls2_sample_next(&iter))
    {
        ...
        /* write DLSID if its set */
        if(sample->dlid)
            if(!dls_write_dlid(writer, sample->dlid, err))
            {
                return (FALSE);
            }
        ...
        if(!dls_write_sample_format(writer, sample, err))
        {
            goto err;
        }
        ...
        if(!writer->is_gig)
        {
            if(!dls_write_info(writer, sample->info, err))
            {
                goto err;
            }
        }
        else if(!gig_write_name_info(writer, sample->info, err))
        {
            goto err;
        }
...
dls_write_dlid(IpatchDLSWriter *writer, guint8 *dlid, GError **err)
{
    ...
    if(!ipatch_file_write(riff->handle, dlid, 16, err))
    {
        return (FALSE);
    }
                         ...
                         GError **err)
{
    ...
    if(i)
    {
        ipatch_file_buf_zero(riff->handle, i);
    }
    ...
    if(i)
    {
        ipatch_file_buf_memset(riff->handle, 0xFF, i);
    }
