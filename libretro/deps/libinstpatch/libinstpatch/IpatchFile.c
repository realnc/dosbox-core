...
ipatch_file_pool_new(const char *file_name, gboolean *created)
{
    ...
    if(created)
    {
        *created = FALSE;    // Initialize in case of bail out..
    }
    ...
    if(lookup)
    {
        ...
        if(!lookup_file)
        {
            g_weak_ref_set(lookup, file);    // !! Re-use weak reference (it was NULL)
        }
    }
    else
    {
        g_hash_table_insert(ipatch_file_pool, abs_filename, weakref);    // !! hash takes over filename and weakref
    }
    ...
    if(!lookup_file)
    {
        if(++createCount >= IPATCH_FILE_POOL_CREATE_COUNT_CLEANUP)  // Garbage collection for destroyed file objects
        {
            ...
            while(g_hash_table_iter_next(&iter, &key, &value))
            {
                ...
                if(value_file)
                {
                    g_object_unref(value_file);    // -- unref file value
                }
                else
                {
                    g_hash_table_iter_remove(&iter);    // Weak reference empty (file object destroyed) - remove
                }
            }
        }
    }
...
ipatch_file_get_refs_by_type(IpatchFile *file, GType type)
{
    ...
    {
        type = G_TYPE_NONE;    // G_TYPE_OBJECT is equivalent to G_TYPE_NONE (i.e., all objects)
    }
    ...
    while(g_hash_table_iter_next(&iter, &key, (gpointer *)&weakref))
    {
        ...
        if(refobj)          // Object still alive?
        {
            ...
            {
                g_object_unref(refobj);    // -- unref object (did not match type criteria)
            }
        }
        else
        {
            g_hash_table_iter_remove(&iter);    // Object destroyed - remove from hash
        }
    }
...
ipatch_file_rename(IpatchFile *file, const char *new_name, GError **err)
{
    ...
    if(log_if_fail(file->iochan == NULL))
    {
        goto error;
    }

    if(log_if_fail(file->file_name != NULL))
    {
        goto error;
    }
                 ...
                 GError **err)
{
    ...
    if(!retval)
    {
        ...
        if(iochan)
        {
            g_io_channel_unref(iochan);    /* -- unref iochan */
        }
...
ipatch_file_assign_io_channel(IpatchFile *file, GIOChannel *iochan)
{
    ...
    if(iochan)
    {
        g_io_channel_ref(iochan);    /* ++ ref for file */
    }
...
ipatch_file_close(IpatchFileHandle *handle)
{
    ...
    if(handle->buf)
    {
        g_byte_array_free(handle->buf, TRUE);
    }
...
ipatch_file_real_identify(IpatchFile *file, gboolean byext, GError **err)
{
    ...
    for(p = children; p && *p; p++)
    {
        ...
        if(file_class->identify)
        {
            ...
            if(handle)
            {
                ipatch_file_seek(handle, 0, G_SEEK_SET, NULL);
            }
        }
...
type_all_children(GType type, GArray *pass_array)
{
    ...
    if(!array)
    {
        array = g_array_new(TRUE, FALSE, sizeof(GType));    /* Zero terminated */
    }
...
ipatch_file_set_iofuncs(IpatchFile *file, const IpatchFileIOFuncs *funcs)
{
    ...
    if(dupfuncs)
    {
        ipatch_item_set_flags(file, IPATCH_FILE_FLAG_FREE_IOFUNCS);
    }
    else
    {
        ipatch_item_clear_flags(file, IPATCH_FILE_FLAG_FREE_IOFUNCS);
