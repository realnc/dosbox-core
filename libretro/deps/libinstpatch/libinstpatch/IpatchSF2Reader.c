...
ipatch_sf2_reader_new(IpatchFileHandle *handle)
{
    ...
    if(handle)
    {
        ipatch_sf2_reader_set_file_handle(reader, handle);
    }
                    ...
                    IpatchSF2GenAmount *amount)
{
    ...
    *genid = ipatch_file_buf_read_u16(handle);
    ...
    {
        amount->sword = ipatch_file_buf_read_s16(handle);
    }
}
...
ipatch_sf2_load_level_0(IpatchSF2Reader *reader, GError **err)
{
    ...
    if(!sfload_infos(reader, err))
    {
        return (FALSE);
    }

    if(!ipatch_riff_end_chunk(riff, err))
    {
        return (FALSE);
    }
    ...
    if(!sfload_shdrs(reader, err))
    {
        return (FALSE);
    }

    /* return to preset bag chunk */
    if(!ipatch_riff_pop_state(riff, err))
    {
        return (FALSE);
    }

    if(!sfload_pbags(reader, err))
    {
        return (FALSE);
    }

    if(!sfload_pmods(reader, err))
    {
        return (FALSE);
    }

    if(!sfload_pgens(reader, err))
    {
        return (FALSE);
    }

    /* skip ihdrs (already loaded above) */
    if(!ipatch_riff_skip_chunks(riff, 1, err))
    {
        return (FALSE);
    }

    if(!sfload_ibags(reader, err))
    {
        return (FALSE);
    }

    if(!sfload_imods(reader, err))
    {
        return (FALSE);
    }

    if(!sfload_igens(reader, err))
    {
        return (FALSE);
    }
    ...
    while(p)			/* traverse through all presets */
    {
        ...
        while(p2)		/* traverse preset's zones */
        {
            ...
            while(i-- > 0)	/* load zone's generators */
            {
                ...
                if(genid == IPATCH_SF2_GEN_NOTE_RANGE)
                {
                    level = 1;
                }
                else if(genid == IPATCH_SF2_GEN_VELOCITY_RANGE)
                {
                    level = 2;
                }
                else if(genid == IPATCH_SF2_GEN_INSTRUMENT_ID)
                {
                    ...
                    if(index >= reader->inst_count)
                        ...
                        ipatch_sf2_zone_set_link_item(zone, (IpatchItem *)
...
sfload_ihdrs(IpatchSF2Reader *reader, GError **err)
{
    ...
    if(chunk->size == 0)
    {
        return (TRUE);    /* no instrument headers? */
    }
...
sfload_shdrs(IpatchSF2Reader *reader, GError **err)
{
    ...
    if(chunk->size <= IPATCH_SFONT_SHDR_SIZE)
    {
