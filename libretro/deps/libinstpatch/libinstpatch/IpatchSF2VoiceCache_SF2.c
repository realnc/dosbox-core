                                       ...
                                       GError **err)
{
    ...
    for(p = preset->zones; p; p = p->next)	/* loop over preset zones */
    {
        ...
        if(solo_item && (IpatchItem *)pzone != solo_item)
        {
            continue;
        }
        ...
        for(; p2; p2 = p2->next)		/* loop over instrument zones */
        {
            ...
            if(cache->override_mods)
            {
                ...
                if(tmpmods)
                {
                    ipatch_sf2_mod_list_free(tmpmods, TRUE);
                }
            }
            ...
            if(imods != gimods)
            {
                ipatch_sf2_mod_list_free(imods, FALSE);
            }
        }
        ...
        if(gimods != defmods)
        {
            ipatch_sf2_mod_list_free(gimods, FALSE);
        }

        /* free pmods list if not gpmods (mod data was not copied) */
        if(pmods != gpmods)
        {
            ipatch_sf2_mod_list_free(pmods, FALSE);
        }
    }
...
_sf2_inst_to_sf2_voice_cache_convert(IpatchConverter *converter, GError **err)
{
    ...
    for(; p; p = p->next)
    {
        ...
        /* If a zone is currently solo, skip other zones */
        if(solo_item && (IpatchItem *)izone != solo_item)
        {
            continue;
        }
        ...
        {
            voice->mod_list = ipatch_sf2_mod_list_override(gimods, izone->mods, TRUE);
        }
