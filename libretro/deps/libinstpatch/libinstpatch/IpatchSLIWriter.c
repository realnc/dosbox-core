...
ipatch_sli_writer_new(IpatchFileHandle *handle, IpatchSLI *sli)
{
    ...
    if(handle)
    {
        ipatch_sli_writer_set_file_handle(writer, handle);
    }

    if(sli)
    {
        ipatch_sli_writer_set_patch(writer, sli);
    }
...
ipatch_sli_writer_set_file_handle(IpatchSLIWriter *writer, IpatchFileHandle *handle)
{
    ...
    if(writer->handle)
    {
        ipatch_file_close(writer->handle);
    }
...
ipatch_sli_writer_save(IpatchSLIWriter *writer, GError **err)
{
    ...
    if(writer->sli)
    {
        g_object_unref(writer->sli);    /* shouldn't be set, but.. */
    }
