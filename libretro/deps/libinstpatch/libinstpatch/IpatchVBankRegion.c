...
ipatch_vbank_region_get_title(IpatchVBankRegion *region, GValue *value)
{
    ...
    {
        g_value_set_static_string(value, _("<Unresolved>"));
    }
}
...
ipatch_vbank_region_first(IpatchIter *iter)
{
    ...
    if(obj)
    {
        return (IPATCH_VBANK_REGION(obj));
    }
    else
    {
        return (NULL);
                                      ...
                                      char **id_props, gboolean notify)
{
    ...
    if(notify)
    {
        g_object_notify(G_OBJECT(region), "id-props");
    }
}
...
ipatch_vbank_region_get_id_props(IpatchVBankRegion *region, guint *n_elements)
{
    ...
    if(n_elements)
    {
        *n_elements = id_props ? g_strv_length(id_props) : 0;
    }
                                  ...
                                  gboolean sample_notify)
{
    ...
    if(item)
    {
        g_object_ref(item);    // !! ref item for region
    }
    ...
    if(olditem)
    {
        g_object_unref(olditem);    // !! unref old item (no longer referenced by region)
    }
...
ipatch_vbank_region_get_item(IpatchVBankRegion *region)
{
    ...
    if(item)
    {
        g_object_ref(item);    // ++ reference for caller
    }
