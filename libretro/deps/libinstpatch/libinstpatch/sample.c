...
TRANS_FUNC(swap64, guint64, guint64, guint64 t;,
{
    ...
    | ((t & G_GINT64_CONSTANT(0xFF00000000000000U)) >> 56);
}, NOP)
...
TRANS_FUNC(sle3bto4b, guint8, guint32, guint i2 = 0;,
{
    ...
    | ((inp[i2 + 2] & 0x80) ? 0xFF000000 : 0); i2 += 3;
}, NOP)
                                  ...
                                  IpatchSampleTransformFunc *funcs)
{
    ...
    if(G_UNLIKELY(dchan < schan))
    {
        ...
        {
            funcs[func_index++] = chanmap_tff[swidth - 1];
        }
             ...
             != IPATCH_SAMPLE_FORMAT_IS_SIGNED(dest_format)))
    {
        ...
        {
            funcs[func_index++] = sign_tff[swidth - 1];
        }
    ...
    if(G_UNLIKELY(dchan > schan))
    {
        ...
        {
            funcs[func_index++] = chanmap_tff[dwidth - 1];    /* Arbitrary channel mapping */
        }
    ...
    if(buf1_max_frame)
    {
        *buf1_max_frame = max[0];
    }

    if(buf2_max_frame)
    {
        *buf2_max_frame = max[1];
    }
