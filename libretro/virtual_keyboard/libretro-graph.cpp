...
void draw_fbox_bmp(unsigned short *buffer, int x, int y, int dx, int dy, uint32_t color, libretro_graph_alpha_t alpha)
{
   ...
   switch (alpha)
   {
      ...
      case GRAPH_ALPHA_25:
         for (j=y; j<y+dy; j++)
         {
            unsigned short *buf_ptr = buffer + (j * RDOSGFXwidth) + x;
            for (i=x; i<x+dx; i++)
            {
      ...
      uint32_t fg, uint32_t bg, libretro_graph_bg_t draw_bg)
{
   ...
   switch (draw_bg)
   {
      case GRAPH_BG_ALL:
         for (yrepeat = y - yscale; yrepeat < surfh + y - yscale; yrepeat++)
         {
            surf_ptr32 = surf + (yrepeat * RDOSGFXwidth) + x - xscale;
      ...
      default:
         for (yrepeat = y - yscale; yrepeat < surfh + y - yscale; yrepeat++)
         {
            surf_ptr32 = (uint32_t *)surf + (yrepeat * RDOSGFXwidth) + x - xscale;
            for (xrepeat = x; xrepeat < surfw + x; xrepeat++, yptr32++)
            {
               if (*yptr32 != 0)
                  *surf_ptr32 = *yptr32;
